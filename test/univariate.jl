...
function LogDensityProblems.logdensity(
    ...
    return logpdf(Normal(zero(F), one(F)), μ) + sum(Base.Fix1(logpdf, Normal(μ, one(F))), y)
end

function SliceSampling.initial_sample(rng::Random.AbstractRNG, model::UniModel)
    return randn(rng, LogDensityProblems.dimension(model))
end
...
@testset "multivariate samplers" begin
    model = UniModel([0.0])
    @testset for sampler in [Slice(1), SliceSteppingOut(1), SliceDoublingOut(1)]
        @testset "initialization" begin
            ...
            θ0    = [1.0]
            chain = sample(model, sampler, 10; initial_params=θ0, progress=false)
